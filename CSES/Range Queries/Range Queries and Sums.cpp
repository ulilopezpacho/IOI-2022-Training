...
struct Node {
    ...
    tint evaluate (int l, int r) {
        ...
        if (setValueLazy > 0) return abarca * setValueLazy + abarca * sumaLazy;
        else return abarca * sumaLazy + suma;
    }
...
struct LazySegmentTree {
    ...
    tint query (int a, int b) {
        return query(a,b,1,0,segN-1);
    }
...
ostream& operator<< (ostream &os, Node &node) {
    os << "(Suma: " << node.suma << ", Suma Lazy: " << node.sumaLazy << ", Set Lazy: " << node.setValueLazy << ")";
    return os; 
}
